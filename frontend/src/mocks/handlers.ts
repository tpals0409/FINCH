 ...
 *
 * 경로는 `*` 로 시작한다. 오리진 설정(`VITE_API_BASE_URL`)이 절대 주소든 상대 주소든
 * 같은 핸들러가 잡히게 하기 위해서다.
 */

/**
 * 인가 코드 접두사로 응답을 고른다. 프론트 혼자 세 갈래를 다 확인할 수 있다.
 *
 * - `fail...` → 카카오 인증 실패 (401 `AUTH_KAKAO_FAILED`)
 * - `new...`  → 최초 로그인 (`isNewUser: true`)
 * - 그 밖     → 기존 회원 로그인
 */
...
const authHandlers = [
  http.post('*/api/v1/auth/kakao', async ({ request }) => {
    ...
    /**
     * `Set-Cookie` 는 흉내만 낸다. 서비스 워커가 내려보내는 응답이라 실제
     * `HttpOnly` 쿠키가 브라우저에 저장되지는 않는다. 프론트는 이 쿠키를 읽지 않으므로
     * 로그인 화면 검증에는 지장이 없지만, **재발급은 목으로 끝까지 확인할 수 없다.**
     */
    return HttpResponse.json(
      {
