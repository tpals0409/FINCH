 ...
 *
 * 읽는 이름이 `--character-*` 다. 애플 방향의 `--finch-*` 를 한 번도 읽지
 * 않는다 — 한쪽을 지울 때 다른 쪽 차트가 무채색으로 죽으면 안 된다.
 */
function readChartPalette(): ChartPalette {
  ...
  return {
    surface: read('--character-surface', '#ffffff'),
    text: read('--character-text', '#2b2622'),
    textMuted: read('--character-text-muted', '#6e645a'),
    border: read('--character-border', 'rgb(43 38 34 / 0.1)'),
    rise: read('--character-rise', '#c0272d'),
    fall: read('--character-fall', '#1b4f9c'),
  };
}
...
/**
 * 일봉 캔들과 거래량. props 는 애플 방향과 같다.
 *
 * 배경은 지면이 아니라 **표면**이다. 이 차트는 흰 카드 안에 있고, 캔버스가
 * 자기 배경을 따로 칠하므로 카드 색과 맞추지 않으면 차트 자리에 크림색
 * 사각이 뚫린다.
 *
 * 캔들 색은 상승 적색, 하락 청색이다. 캐릭터 방향이라고 캔들을 파스텔로
 * 옅히지 않는다 — 차트는 이 화면에서 가장 밀도가 높은 정보이고, 옅은 적/청
 * 두 색이 겹치면 어느 쪽이 상승인지 읽는 데 실패한다.
 *
...
export function StockCandleChart({ candles, height }: StockCandleChartProps) {
  ...
  useEffect(() => {
    ...
    const candleSeries = chart.addSeries(CandlestickSeries, {
      ...
      // 마지막 종가 축 라벨을 끈다. 기본값은 등락색으로 채운 알약 배지를
      // 축에 그리는데, 신호색을 배경으로 쓰지 않는다는 규칙에 걸리고 화면
      // 최대 활자인 현재가와 같은 숫자를 두 번 말한다.
  ...
  useEffect(() => {
    ...
    // 차트 아래에 깔려 캔들과 경쟁한다. 거래량은 방향이 아니라 양이다.
