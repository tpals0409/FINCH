...
7. 다른 파트에 물어야 할 것을 MR 본문에만 적지 않는다. MR 본문은 머지되면 아무도 다시 읽지 않는다. `frontend/docs/contracts.md`의 미확정 항목으로 추가하고 `_inbox/`에 발송 대기 파일을 만든 뒤, worker_done에 "새 질문 N건, 경로 ..."를 명시한다.
8. 구두 합의는 확정으로 올리지 않는다. 문서에 반영되기 전까지 미확정이다. 회의에서 정해졌다는 말만으로 `contracts.md`의 확정 표에 넣지 않는다. 잠정 확정으로 올리려면 값·근거·틀렸을 때 고칠 범위를 함께 적는다.
