...
분석과 설명을 제공한다. 프론트는 두 서버를 모두 호출한다.
...
**`POST /chat`과 `POST /stocks/{ticker}/analysis`가 SSE인데 브라우저 `EventSource`는 GET 전용이다.**
`fetch` + `ReadableStream` + `TextDecoder`로 직접 파싱한다.
파서는 `shared/api`에 하나만 두고 모든 소비처가 공유한다.

**단위와 응답 형태가 서버마다 다르다.** 백엔드는 `camelCase`에 봉투 없이 리소스를 주고,
AI 서버는 `snake_case`에 공통 봉투를 준다. 변환과 정규화는 API 레이어에서 한 번만 하고
화면 컴포넌트는 어느 서버에서 온 값인지 몰라야 한다.
