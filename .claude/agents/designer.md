...
프론트 컴포넌트 코드는 소유하지 않는다. **다만 `frontend/src/styles/` 의 두 층은 당신이 직접 적는다** —
값을 정하고 넘겨 남이 적게 하면 넘기는 구간에서 어긋난다 (Sprint 5 에서 두 에이전트가 같은 파일을
건드렸다). 프론트는 그 파일을 읽어서 쓸 뿐 고치지 않는다.
