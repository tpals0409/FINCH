...
**Base** /api/ai/v1 · **Auth** 백엔드 신뢰 헤더 (내부 전용) · **Format** JSON · UTF-8 · **Runtime** FastAPI (별도 서비스)
...
| --- | --- |
| Base URL | `/api/ai/v1` |
| 인증 | `X-User-Id: <사용자 식별자>` — 백엔드가 JWT를 검증한 뒤 넣어 주는 신뢰 헤더다. AI 서버는 JWT를 다시 검증하지 않으며, 사용자 식별자를 **경로·본문에 넣지 않는다**. 값을 검증 없이 신뢰하므로 **AI 서버는 내부 네트워크에서만 접근 가능해야 한다** |
...
| 401 | `UNAUTHORIZED` | 백엔드 신뢰 헤더 누락·공백 | 재로그인 |
