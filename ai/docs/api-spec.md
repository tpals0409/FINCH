...
요청 한도는 `X-User-Id`와 엔드포인트별로 독립 집계한다. 초과하면 `detail.reason=request_rate_limit`, `Retry-After` 헤더와 대기 시간을 반환한다. 실제 GMS 호출 직전에는 사용자별 일일 토큰을 예약한다. 캐시가 응답을 대신하면 예약하지 않는다. 일일 예산을 넘으면 `detail.reason=daily_token_budget`으로 429를 반환한다. 기본 예산은 입력·출력 합계 500,000 토큰이며 환경 변수로 조정할 수 있다.
