...
class RequestIdFilterTest {
	...
	void generatesWhenAbsent() throws Exception {
		MvcResult result = mockMvc.perform(get("/stub/echo"))
	...
	void reusesIncoming() throws Exception {
		mockMvc.perform(get("/stub/echo").header(RequestIdFilter.HEADER, "nginx-abc.123_x"))
			.andExpect(status().isOk())
	...
	void rejectsUnsafeIncoming() throws Exception {
		...
		MvcResult result = mockMvc.perform(get("/stub/echo").header(RequestIdFilter.HEADER, unsafe))
			.andExpect(status().isOk())
			.andReturn();
	...
	void clearsMdcAfterRequest() throws Exception {
		mockMvc.perform(get("/stub/echo")).andExpect(status().isOk());
