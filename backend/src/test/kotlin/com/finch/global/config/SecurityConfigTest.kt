 ...
 *
 * 여기서만 볼 수 있는 것이 하나 있다 — **화이트리스트가 실제 필터 체인에서 어떻게 갈리는지**다.
 * `@WebMvcTest` 는 컨트롤러 하나만 올리므로 actuator 경로가 존재하지 않고, 열려 있는지 막혀
...
class SecurityConfigTest {
	 ...
	 * **인증이 먼저 걸린다**는 사실만 기록한다. 프론트가 404 를 기대하는 자리는 인증된 요청뿐이다.
	 */
	@Test
	@DisplayName("없는 경로도 토큰이 없으면 404 가 아니라 401 이다")
	fun unmappedPathIsAuthenticatedFirst() {
	...
	private fun statusOf(path: String): HttpStatusCode =
		client.get().uri(path).exchange { _, response -> response.statusCode }
}
