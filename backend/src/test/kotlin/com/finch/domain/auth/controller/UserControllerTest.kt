 ...
 * - apiSpec 2.4 의 응답 계약 — 필드 이름 하나가 달라지면 프론트의 Zod 파싱이 깨진다.
 * - **인증 실패 코드의 분기** — 프론트 인터셉터는 `AUTH_TOKEN_EXPIRED` 에서만 재발급을
 *   시도하고 `AUTH_INVALID_TOKEN` 에서는 로그인 화면으로 보낸다 (apiSpec 1.2).
 *   두 코드가 뒤바뀌면 무한 재발급 루프이거나, 반대로 로그인이 30분마다 풀린다.
 *
 * `@WebMvcTest` 라 Docker 없이 돈다. 필터 체인은 `@Import(SecurityConfig::class)` 로
 * 실제 것을 그대로 쓰고 `JwtProvider` 만 목이다 — 검사대의 배선을 보는 것이 목적이다.
 */
...
class UserControllerTest {
	...
	fun missingHeaderIsInvalidToken() {
	...
	fun malformedHeaderIsInvalidToken() {
	...
	fun expiredTokenKeepsItsOwnCode() {
	...
	fun tamperedTokenIsInvalidNotExpired() {
		given(jwtProvider.parseAccessToken("tampered"))
			.willThrow(CustomException(AuthErrorCode.AUTH_INVALID_TOKEN))
	 ...
	 * 프론트가 401 본문을 두 가지로 파싱해야 한다. 같은 `ErrorResponse` 를 쓰는지 본다 (apiSpec 1.3).
	 */
	@Test
	@DisplayName("인증 실패 본문도 다른 에러와 같은 형식이다 — code · message 가 있고 detail 은 없다")
	fun authFailureUsesTheSameErrorBody() {
		mockMvc.perform(get("/api/v1/users/me"))
			.andExpect(status().isUnauthorized)
			.andExpect(jsonPath("$.code").value("AUTH_INVALID_TOKEN"))
			.andExpect(jsonPath("$.message").value(AuthErrorCode.AUTH_INVALID_TOKEN.message))
			.andExpect(jsonPath("$.detail").doesNotExist())
			// 인증 실패도 추적 대상이다. RequestIdFilter 는 Security 보다 앞이라 헤더가 남아야 한다.
			.andExpect(header().exists("X-Request-Id"))
	}

	private fun givenLoggedIn(userId: Long) {
	...
	companion object {
		...
		private fun userMeRes(userId: Long): UserMeRes = UserMeRes(
			userId, "홍길동", "https://img.kakao/1.jpg",
