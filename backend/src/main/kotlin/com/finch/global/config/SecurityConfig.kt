...
class SecurityConfig {
	...
	/**
	 * 이 빈이 없으면 Spring Security 기본 설정이 전 경로를 인증 대상으로 잡는다 (아래 PUBLIC_PATHS 주석).
	 *
	 * 인증의 뼈대는 세 조각으로 나뉘어 있고 각자 한 가지만 한다.
	 * 1. [JwtAuthenticationFilter] — 헤더를 읽어 사용자를 앉힌다. **거부하지 않는다.**
	 * 2. `AuthorizationFilter`(스프링 기본) — 아래 경로 규칙으로 거부 여부를 정한다.
	 * 3. [JwtAuthenticationEntryPoint] — 401 본문을 쓴다. 쓰는 곳은 여기 하나다.
	 *
	 * 나눈 이유는 [JwtAuthenticationFilter] 주석에 있다 — 필터가 직접 거부하면 무인증 경로가
	 * 무효한 헤더 때문에 깨지고, 예외를 던지면 `GlobalExceptionHandler` 를 타지 못해 500 이 나간다.
	 *
	 ...
	 *
	 * **CORS 설정을 두지 않는다.** 배포는 nginx 가 `/` → 프론트, `/api/` → 백엔드로 붙여 같은
	...
	fun filterChain(http: HttpSecurity, jwtProvider: JwtProvider, objectMapper: ObjectMapper): SecurityFilterChain =
		http
			...
			.exceptionHandling { handling ->
				handling.authenticationEntryPoint(JwtAuthenticationEntryPoint(objectMapper))
			}
	...
	companion object {
		...
		// Kotlin 의 블록 주석은 Java 와 달리 중첩된다. 아래 본문의 `/actuator/health/**` 가
		// 블록 주석 안에서는 새 주석을 여는 `/**` 로 읽혀 파일 끝까지 주석이 되므로 줄 주석으로 적는다.
		//
		// 인증 없이 열어 두는 경로. 늘릴 때는 **왜 무인증이어야 하는지**를 여기 적는다.
		//
		...
		// **애플리케이션 로그에는 아무 에러도 남지 않아** 원인을 찾기 어렵다.
		// `/actuator/prometheus` — Prometheus scrape 대상이다. 막으면 에러 없이 빈 스크레이프만 계속된다.
		// 둘 다 인증 헤더를 붙일 방법이 없는 호출자다. `SecurityConfigTest` 가 이 세 경로를 못 박는다.
		//
		// 나머지 actuator 엔드포인트(`/actuator/info` 등)는 열지 않는다 — 프로브가 쓰지 않는다.
		//
		// ⚠️ `/internal/v1/**`(AI 서버 전용, apiSpec 9장)은 **일부러 넣지 않았다.** 그 경로의 인증은
		// JWT 가 아니라 `X-Internal-Token` 이고(apiSpec 11.2) 그 검사 코드가 아직 없다. 지금 열어 두면
		// 컨트롤러가 생기는 순간 **인증 없는 내부 API** 가 조용히 공개된다. 닫아 두면 첫 호출이 401 로
