...
/** 활성 종목을 정렬된 커서로 순회하며 KIS 현재가를 Redis에 적재한다. */
@Component
...
internal class KisPriceCollector internal constructor(
	...
	) : this(client, targetRepository, cacheWriter, lease, batchSize, Clock.systemUTC())
	...
	@Scheduled(fixedDelayString = "\${finch.price.kis.cycle-interval}")
	fun collect() {
		if (!isMarketOpen() || clock.instant().isBefore(pausedUntil) || !lease.acquireOrRenew()) return

		var targets = targetRepository.findAfter(cursor, batchSize)
		if (targets.isEmpty() && cursor.isNotEmpty()) {
			cursor = ""
			targets = targetRepository.findAfter(cursor, batchSize)
		}
