...
기존 증권 서비스를 클론하고 AI 투자 비서를 얹은 모의투자 웹 서비스. 팀 프로젝트에서 갈라져 나온 개인 ver2 다. **백엔드가 원장과 시세의 단일 진실 공급원**이고, 프론트는 백엔드만 호출하며,
...
이 저장소에서 도는 개발 에이전트의 이름은 **Fin** 이다. 서버에 상주하는 **Pico** 와 짝이고,
경계는 `kubectl` 이다 — 클러스터 안은 Pico, 저장소와 코드는 Fin.
...
배포에 필요한 비밀값과 절차는 두 문서에 있다 — 무엇을 채워야 하는지는 루트 `keys.md`,
어떤 순서로 올리는지는 `docs/ops/deploy-runbook.md` 다.
...
Fin 은 터미널에서 직접 띄우기도 하고 **Buzz 를 통해 뜨기도 한다.**

```
Buzz Desktop  ──  buzz-acp  ──  claude-agent-acp  ──  로컬 Claude Code
```

**Buzz 에서 띄울 때는 작업 디렉터리를 이 저장소 루트로 잡는다.** 그래야 이 문서와 파트
브리핑이 딸려 온다. 엉뚱한 곳에서 띄우면 규칙 없이 시작하고, 그건 겉으로 드러나지 않는다.

**대화는 세션 사이에 안 넘어간다.** 넘어가는 것은 이 문서와 `~/.claude/projects/` 아래의
메모리뿐이다. 그래서 다음 세션이 알아야 할 것은 대화에 남기지 말고 둘 중 하나에 적는다.
