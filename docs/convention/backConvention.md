...
- KIS 모의·실전 URL은 기본값으로 정하지 않는다. 수집을 켜는 환경은 자격증명 종류와 맞는
  `KIS_BASE_URL`을 명시해 검증한 환경과 실행 환경이 갈리지 않게 한다.
- OAuth 토큰은 프로세스 안에서 만료 1분 전에 갱신하며, KIS `EGW00123` 응답에는 토큰을 비우고 현재가
  요청을 정확히 한 번 다시 보낸다. 토큰 발급의 실측 제한 `EGW00133`(1분당 1회)을 받으면 60초 동안
  수집을 멈춘다. 토큰·앱키·앱시크릿과 원문 응답은 로그에 남기지 않는다.
- 현재가 틱 `price:{stockCode}`에는 TTL을 두지 않는다. 수신이 끊긴 뒤에도 마지막 값을 유지하고
  `stale: true`로만 표시해야 하기 때문이다(apiSpec 5.4). 30초 TTL은 틱이 아니라 관심 신호의 수명이다.
- `stock.previous_close`는 AI가 정한 최근 거래일의 전 종목 종가를 매일 08:30 KST에 받아 한 SQL로
  갱신한다. 백엔드는 거래일 달력을 갖지 않는다. 응답에 없는 종목과 빈 응답은 기존 값을 보존하며,
  값이 달라진 행만 `updated_at`과 함께 바꿔 같은 배치를 다시 실행해도 결과가 같다.
- KIS가 `Retry-After`를 주면 그 시간까지 수집을 멈춘다. 그 밖의 재시도 가능한 오류도 같은 주기 안에서
  자동 반복하지 않고 다음 스케줄로 넘겨 외부 장애 때 호출 폭주를 막는다.
