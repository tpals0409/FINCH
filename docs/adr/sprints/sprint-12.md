...
| 이슈 | 결과 | 근거 |
|---|---|---|
| #102 — KIS 실패 로깅 | ✅ 완료 | PR #112, `6405447`. 실제 로그에서 `KIS 시세 수집 실패 stockCode=005930 retryable=false`, `kisMsgCd=EGW00201` 을 확인했다. **`kisMsg` 를 "종목코드 오류" 로 적었던 것은 오기다 — 실제 값은 "초당 거래건수를 초과하였습니다." 다** (2026-09-10 정정, 아래 참고). |
...
**위 #102 행의 `kisMsg=종목코드 오류` 는 틀렸다. `EGW00201` 은 초당 거래건수 초과다.**

Pico 가 같은 보존 로그를 다시 분리해 세었다 — `kisMsgCd=EGW00201` 598건의 `kisMsg` 는
**한 종류뿐이고 전부 `"초당 거래건수를 초과하였습니다."`** 였다.

이 오기가 하루를 먹었다. 2026-09-10 에 005930 의 `EGW00201` 17건을 보고 "삼성전자에
종목코드 오류가 난다" 로 읽어 파라미터·모의환경 가설을 세웠는데, 실제로는 페이싱 문제였다.
근거로 삼은 두 곳이 서로 독립이 아니었다 — 이 ADR 행과
`KisPriceCollectorTest` 의 픽스처가 같은 문자열이었고, **픽스처 쪽은 사람이 골라 넣은 값**이다.

**교훈: 테스트 픽스처의 문자열은 외부 시스템의 사실이 아니다.** 그리고 그 문자열을 인용한
문서는 독립 근거가 아니다. 외부 시스템의 코드·메시지 대응은 원문 로그에서 확인한다.
