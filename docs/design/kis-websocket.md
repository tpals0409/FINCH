...
상태: T5 운영 실측 대기. 이 문서는 구현 PR이 아니라 백엔드↔KIS 수집 계층의 설계안이다.
...
- KIS 웹소켓 주소, 모의 키의 `approval_key` 발급 가능 여부, 구독 상한은 T5 실측 전에는
  코드나 설정에 넣지 않는다.
...
T5에서 확인한 KIS 모의 웹소켓 주소와 실제 approval key 발급 요청만 설정 계약으로 확정한다.
현재가 REST의 access token을 웹소켓 인증값으로 재사용하지 않는다.
...
5. 연결 확인 후 현재 핫셋을 구독
6. 수신 체결/현재가 프레임을 decode하여 PriceCacheWriter.write()
```
...
구독 상한은 T5에서 측정한 값을 단일 설정으로 주입한다. 상한을 넘는 종목의 우선순위는 다음과
같이 서버 전체 LRU로 결정한다.
...
다만 이 우선순위와 “보유를 항상 보장할지”는 T5에서 구독 상한이 핫셋보다 작을 때 확정한다.
...
RECONNECTING
  ...
  ├─ 성공 → approval_key 재발급 여부를 계약에 따라 판단
...
다음 항목은 현재 설계의 미확정값이다.

1. approval key 발급 성공 여부와 요청 경로·응답의 비밀값 제외 필드
2. 모의 웹소켓 endpoint 연결 성공 여부
3. 005930 구독 후 실제 틱 페이로드의 필드와 시각 의미
4. 앱 키 하나로 가능한 동시 구독 종목 수
5. 웹소켓 자체의 초당 메시지·구독 등록 제한
6. 재연결 시 approval key 재발급이 필요한지
7. 구독 상한이 30종보다 작을 때 보유·관심·최근 본 우선순위

1~5가 충족되지 않으면 이 설계를 구현으로 진행하지 않고, 확인된 제한에 맞춰 REST 핫셋 정책 또는
실전 키 사용 여부를 다시 결정한다. 충족되면 이 문서를 근거로 별도 구현 PR에서 KIS 클라이언트,
조정기, 폴백 분배, 단위·통합 테스트를 추가한다.
