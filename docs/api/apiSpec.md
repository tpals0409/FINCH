...
| 생존 확인 | STOMP 내장 하트비트. 간격과 서버 회수 타임아웃 수치는 `[S0-1]`·`[S0-2]`에서 확정 |
...
서버는 종목별 마지막 요청 시각을 기록해 TTL 경과 시 슬롯을 회수한다. 폴링 중단이 곧 해제다.
...
| 6 | STOMP 하트비트 간격·서버 회수 타임아웃·폴링 티어 TTL 수치 — 채널 자체는 5.6에서 확정 | `[S0-1]`·`[S0-2]` | 5.6 |
