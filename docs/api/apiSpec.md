...
- **깨진 커서는 `400 INVALID_REQUEST`다.** 조용히 첫 페이지를 주면 무한 스크롤이 처음으로 되돌아간다.
- `size`가 범위를 벗어나면 거부하지 않고 **자른다.** `size=200`은 100으로, `size=0`은 1로 처리된다. 잘린 사실은 `items` 길이로 드러난다.

> **인코딩 방식 (구현 참고, 계약이 아니다)** — `{"id":123}`의 Base64URL이다. 숫자만 넣으면 더
> 짧지만, 정렬 축이 둘인 목록(보유 종목의 평가금액순 등)이 생겼을 때 필드를 더할 수 없고 옛 커서와
> 새 커서를 구분할 표식이 없어 이전 커서가 조용히 잘못 페이징된다. **클라이언트는 이 형식에
> 의존하지 않는다** — 여기 적는 것은 서버 로그와 버그 재현을 위한 참고다.
...
목록에서 빼지 않는다 — **이름과 코드는 그리고 가격 자리만 비운다.** 시세 수집 전에는 전 종목이,
수집 후에는 아직 보유·관심·최근 본 핫셋에 들어오지 않은 종목이 이 상태일 수 있다.
...
- 백엔드는 AI 내부 API에서 전역 현재가 핫셋(최대 30종)의 최근 1년 수정주가 OHLCV를 매일
  08:00 KST에 읽어 `daily_candle`에 멱등 적재한다. 종목 하나가 실패해도 나머지 종목은 계속한다.
- 배열은 `date` 오름차순이다. 동기화 전이거나 원천에 값이 없으면 `candles: []`를 반환한다.
- `1M`·`3M`·`1Y` 경계는 KST 오늘에서 각각 30·90·365 달력일을 뺀 날짜부터다.
...
관심도 상위 종목만 KIS 실시간에 등록하고, 백엔드 KIS REST 수집은 **보유(`quantity > 0`) ∪ 관심 ∪ 최근 본**
핫셋을 하나의 시세 캐시에 동일 스키마로 적재한다. 핫셋 밖 종목은 캐시 미스가 허용되고 §5.4의 값 없음 응답을
사용한다. 프론트는 종목이 어느 티어인지 알 필요가 없으며 신선도 차이는 `asOf`와 `stale`로만 드러난다.
...
아래 값 중 KIS 호출 간격과 한 주기 호출 수는 코드 기본값이 아니다. 초기 운영은 모의 한도에도 안전한
500ms·6건을 `KIS_MIN_REQUEST_INTERVAL`과 `KIS_PRICE_BATCH_SIZE`로 같은 배포 변경에 주입한다. 이는
한도 실측값이 아니라 보수적으로 선택한 값이며, 핫셋이 30종목을 넘으면 `[S0-1]` 실측 뒤 조정한다.
**수치를 조정하더라도 관계식은 유지한다.**
...
| KIS REST 호출 간격 | 필수 주입값 `KIS_MIN_REQUEST_INTERVAL` (초기 운영 500ms) |
| 한 주기 REST 호출 수 | 필수 주입값 `KIS_PRICE_BATCH_SIZE` (초기 운영 6건, `주기 ÷ 호출 간격` 이하) |
...
4. **REST 핫셋 수용량 = KIS REST 초당 호출 한도 × `stale` 허용 시간** — 예상 한 바퀴가 허용 시간을
   넘으면 백엔드는 WARN을 남긴다. 우선순위 큐나 임계 변경은 실제 초과를 측정한 뒤 결정한다
...
- `REGISTERED` 는 **담은 순서의 역순이다 — 최근에 담은 것이 위다.**
- `CHANGE_RATE` 는 내림차순이고 **시세 없는 종목(`changeRate: null`)은 맨 뒤다.**
...
**`type=DEPOSIT`은 원장 유형 `DEPOSIT`(충전)만이다. 초기 지급은 포함하지 않는다.** 이 필터의 합계가
`GET /deposits/limit`의 `depositedAmount`(= 충전액 누적, 초기 지급 제외)와 같아야 하기 때문이다.
둘이 어긋나면 "충전 내역을 다 더했는데 한도 화면의 숫자와 다르다"가 된다.

**`INITIAL_GRANT` 1건은 `type=ALL`에서 나온다.** 계정당 정확히 1건이고 가장 오래된 행이므로 목록의
맨 끝(마지막 페이지)에 있다. 어느 필터에서도 볼 수 없는 행이 아니다.

화면의 필터 이름은 **"충전"**이다(명세 8장). "입금"으로 부르면 사용자가 초기 지급 1,000,000원도
그 안에 있을 것으로 기대하는데 실제로는 없다 — 이름이 계약과 어긋나는 자리다.
...
- 필드 최종 확정: 현재 원본 값 계약으로 확정한다. 평가손익 등 파생 지표는 AI가 계산한다.
...
**`AUTH_` 접두는 사용자 인증 전용이고 백엔드만 발행한다. AI 서버는 이 접두의 코드를 내지 않는다.**
프론트가 재로그인 인터셉터를 이 접두로 거는 전제이므로(프론트 contracts C23·C67·C68), AI 중계
경로에서 401 이 올라와도 그 코드는 `AUTH_` 가 아니다 — 사용자 토큰과 무관한 실패이기 때문이다.
AI 중계 에러의 코드 소유는 §10.4 를 따른다.
...
| 4 | AI 응답의 파생 지표는 백엔드가 포함하지 않고 AI가 계산한다 | 완료 | 9.1, 9.2 |
