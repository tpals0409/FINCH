...
> 0-5 공통 API 규격(옛 팀 이슈 23)이 다르게 확정되면 AI 파트가 맞춥니다.
> 백지에서 설계하는 대신 이걸 놓고 고치는 편이 빠를 것 같아 먼저 올립니다.
>
> 관련: 옛 팀 이슈 70 · 선행 대상 옛 팀 이슈 23 · 후속 옛 팀 이슈 30
...
이게 3-4(옛 팀 이슈 56)의 본체이므로 코드 체계가 바뀌면 그쪽도 같이 바뀝니다.
...
4·5번은 AI 파트에서 확정해 구현까지 마쳤습니다(옛 팀 이슈 30). **§4 의 배포 조건(AI 서버 외부 비노출)** 만 인프라 쪽에서 확인해 주시면 됩니다.
